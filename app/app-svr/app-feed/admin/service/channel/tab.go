...
func (s *Service) AddTab(c context.Context, param *show.ChannelTabAP, name string, uid int64) (err error) {
	if err = s.IsValid(0, param.TagID, param.Stime, param.Etime, param.Priority, param.IsNew); err != nil {
		return
	}
...
func (s *Service) IsValid(id, tagID, sTime int64, eTime int64, priority, IsNew int) (err error) {
	var (
		count int
	)
	if sTime > eTime {
	...
	if count, err = s.showDao.ChannelTabValid(id, tagID, sTime, eTime, priority, IsNew); err != nil {
		return
	}
	if count > 0 {
	...
	if count, err = s.showDao.ChannelTabValid(id, tagID, sTime, eTime, 0, IsNew); err != nil {
		return
	}
	//nolint:gomnd
	if count >= 3 {
...
func (s *Service) UpdateTab(c context.Context, param *show.ChannelTabUP, name string, uid int64) (err error) {
	if err = s.IsValid(param.ID, param.TagID, param.Stime, param.Etime, param.Priority, param.IsNew); err != nil {
		return
	}
