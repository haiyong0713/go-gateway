...
var (
	...
	_regionlimit = map[int8]map[string]map[int]string{
		model.PlatIPhone: {
			"65542_bilibili://cliparea": {
				5960: "gt",
				6570: "lt",
			},
			"65541_bilibili://category/65541": {
				5960: "gt",
				6570: "lt",
			},
			"65544_bilibili://albumarea": {
				6090: "gt",
				6570: "lt",
			},
		},
...
Retry:
	for _, rtmp := range rs {
		...
		*r = *rtmp
		...
		if auth, ok := auths[r.Area]; ok && auth.Play == int64(locmdl.Status_Forbidden) {
...
// nolint:gocognit
func (s *Service) NewRegionList(c context.Context, plat int8, build, teenagersMode int, ver, mobiApp, _, language string) (res []*region.Region, version string, err error) {
	ip := metadata.String(c, metadata.RemoteIP)
	var (
		hantlanguage = "hant"
	)
