...
func TestService_HelpList(t *testing.T) {
	Convey("test help HelpList", t, WithService(func(s *Service) {
		...
		version := 1
		res, err := s.HelpList(context.Background(), pTypeID, version)
		So(err, ShouldBeNil)
...
func TestService_HelpDetail(t *testing.T) {
	Convey("test help HelpDetail", t, WithService(func(s *Service) {
		...
		version := 1
		resD, resL, total, err := s.HelpDetail(context.Background(), fID, qTypeID, 0, 1, 15, version)
		So(err, ShouldBeNil)
...
func TestService_HelpSearch(t *testing.T) {
	Convey("test help HelpSearch", t, WithService(func(s *Service) {
		...
		keyWords := "aaa"
		version := 1
		res, total, err := s.HelpSearch(context.Background(), pTypeID, keyWords, 0, 1, 15, version)
		So(err, ShouldBeNil)
		So(total, ShouldBeGreaterThan, 0)
		So(len(res), ShouldBeGreaterThan, 0)
		Println(len(res))
