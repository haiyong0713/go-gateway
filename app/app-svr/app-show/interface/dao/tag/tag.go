...
// TagMsg .
func (d *Dao) TagMsg(c context.Context, tagId, mid int64) (*taggrpc.Tag, error) {
	rly, e := d.tagGRPC.Tag(c, &taggrpc.TagReq{Tid: tagId, Mid: mid})
	if e != nil {
		log.Error("d.tagGRPC.Tag(%d) error(%v)", tagId, e)
		return nil, e
	}
	if rly != nil {
		return rly.Tag, nil
...
func (d *Dao) TagInfo(c context.Context, mid int64, tagId int, now time.Time) (data *tag.Tag, err error) {
	...
	if res.Data == nil {
		err = errors.New("TagInfo list is nil")
		return
	}
	data = res.Data
	return
...
func (d *Dao) TagHotsId(c context.Context, rid int, now time.Time) (tags []*tag.Tag, err error) {
	...
	for _, t := range res.Data[0].Tags {
		if t == nil {
			err = errors.New("TagHotsId struct is nil")
			return
		}
	}
...
func (d *Dao) TagArchive(c context.Context, aid int64) (data []*tag.TagOld, err error) {
	params := url.Values{}
	params.Set("aid", strconv.FormatInt(aid, 10))
	var res struct {
		Code int           `json:"code"`
		Data []*tag.TagOld `json:"data"`
	}
