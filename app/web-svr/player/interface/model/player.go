...
type OperationCard struct {
	...
	StandardCard *OperationStandardCard `json:"standard_card,omitempty"`
	SkipCard     *OperationSkipCard     `json:"skip_card,omitempty"`
	BizType      int32                  `json:"biz_type"`
	ParamFollow  *ParamFollow           `json:"param_follow,omitempty"`
	ParamReserve *ParamReserve          `json:"param_reserve,omitempty"`
	ParamJump    *ParamJump             `json:"param_jump,omitempty"`
}
...
type ParamReserve struct {
	ActivityID int64 `json:"activity_id"`
}

type ParamJump struct {
	URL string `json:"url"`
}

type OperationStandardCard struct {
	Title               string `json:"title"`
	ButtonTitle         string `json:"button_title"`
	ButtonSelectedTitle string `json:"button_selected_title"`
	ShowSelected        bool   `json:"show_selected"`
}

type OperationCustomCard struct {
	ImgURL         string `json:"img_url"`
	SelectedImgURL string `json:"selected_img_url"`
	JumpURL        string `json:"jump_url"`
	ShowSelected   bool   `json:"show_selected"`
}

type OperationSkipCard struct {
	From    int32  `json:"from"`
	To      int32  `json:"to"`
	Icon    string `json:"icon"`
	Label   string `json:"label"`
...
type Player struct {
	...
	// Interaction
	Interaction template.HTML
	// pugv
	PugvWatchStatus  int32
	PugvPayStatus    int32
	PugvSeasonStatus int32
	// pcdn
	Pcdn       template.HTML
	PcdnLoader template.HTML
	// options
	Options template.HTML
	// guide attention
	GuideAttention template.HTML
	JumpCard       template.HTML
	OperationCard  template.HTML
}
...
<is_pay_preview>{{.IsPayPreview}}</is_pay_preview>
<preview_toast>{{.PreviewToast}}</preview_toast>
<interaction>{{.Interaction}}</interaction>
<pugv_watch_status>{{.PugvWatchStatus}}</pugv_watch_status>
<pugv_pay_status>{{.PugvPayStatus}}</pugv_pay_status>
<pugv_season_status>{{.PugvSeasonStatus}}</pugv_season_status>
<pcdn>{{.Pcdn}}</pcdn>
<pcdn_loader>{{.PcdnLoader}}</pcdn_loader>
<options>{{.Options}}</options>
<guide_attention>{{.GuideAttention}}</guide_attention>
<jump_card>{{.JumpCard}}</jump_card>
<operation_card>{{.OperationCard}}</operation_card>
`
