...
import (
	...
	"go-gateway/app/app-svr/macross/service/model/mail"
	"io/ioutil"
)
...
func sendmail(c *bm.Context) {
	...
	if m.Subject == "" {
		res["message"] = fmt.Sprintf("mail title can not be empty!")
		c.JSONMap(res, ecode.RequestErr)
		return
	}
	if m.Body == "" {
		res["message"] = fmt.Sprintf("mail content can not be empty!")
		c.JSONMap(res, ecode.RequestErr)
		return
	}
	if len(m.ToAddresses) == 0 {
		res["message"] = fmt.Sprintf("mail address list can not be empty!")
		c.JSONMap(res, ecode.RequestErr)
		return
	}
