...
import (
	...
	model "go-gateway/app/app-svr/app-interface/interface-legacy/model/relation"
...
func (s *Service) Followings(c context.Context, vmid, mid int64, pn, ps int, version uint64, order string) (f []*model.Following, crc32v uint32, total int, err error) {
	...
	var (
		flm   map[int64]*relationgrpc.FollowingReply
	...
	for _, r := range res {
		fl := &model.Following{MFollow: &model.MFollow{}}
		fl.MFollow.ConverFollow(r)
		if _, ok := cardm[r.Mid]; ok {
			...
			if mid != vmid {
				if f, ok := flm[r.Mid]; ok {
					...
					fl.Attribute = model.AttrNoRelation
				}
			}
			fl.OfficialVerify = cardm[r.Mid].Official
			//nolint:gomnd
			if fl.OfficialVerify.Role == 7 {
				fl.OfficialVerify.Role = 1
			}
...
func (s *Service) Tag(c context.Context, mid, tid int64, pn, ps int) (t []*model.Tag, err error) {
	...
	for _, mid = range mids {
		...
		if _, ok := cardm[mid]; ok {
			...
			//nolint:gomnd
			if tag.OfficialVerify.Role == 7 {
				tag.OfficialVerify.Role = 1
			}
