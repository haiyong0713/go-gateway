...
func (s *Service) PasterAPP(c context.Context, plat, adType int8, aid, typeID, buvid string) (res *model.Paster, err error) {
	...
	var (
		args    = &api.PageRequest{Aid: faid}
		pageTmp *api.PageReply
	)
	if pageTmp, err = s.arcGRPC.Page(c, args); err != nil {
		log.Error("s.arcRPC.Page3(%d) error(%v)", faid, err)
		return
	}
	if pages = pageTmp.GetPages(); len(pages) == 0 {
		return
	}
	res.AID = faid
	res.CID = pages[0].Cid
...
func (s *Service) PasterPGC(c context.Context, plat, adType int8, sid string) (res *model.Paster, err error) {
	...
	var (
		args     = &api.PageRequest{Aid: faid}
		pagesTmp *api.PageReply
	)
	if pagesTmp, err = s.arcGRPC.Page(c, args); err != nil {
		log.Error("PasterPGC s.arcGRPC.Page(%d) error(%v)", faid, err)
		return
	}
	if pages = pagesTmp.GetPages(); len(pages) == 0 {
		return
	}
	res.AID = faid
	res.CID = pages[0].Cid
...
func (s *Service) PasterCID(c context.Context) (res map[int64]int64, err error) {
	...
	for _, aidm := range s.PasterAIDCache {
		...
		if len(aids) > 0 {
			g.Go(func() (err error) {
				var (
					args    = &api.ArcsRequest{Aids: aids}
					arcsTmp *api.ArcsReply
				)
				if arcsTmp, err = s.arcGRPC.Arcs(ctx, args); err != nil {
					log.Error("%v", err)
					err = nil
					return
				}
