...
func FromPlayViewReq(req *v2.PlayViewReq) *v2.ChronosPkgReq {
	return &v2.ChronosPkgReq{
		Aid:      req.Aid,
		Cid:      req.Cid,
		Mid:      req.Mid,
		MobiApp:  req.MobiApp,
...
// nolint:gosimple
func FormatPlayRule(in *PlayerRule) *PlayerReply {
	rly := &PlayerReply{Title: in.Title, File: in.File, MD5: in.MD5}
	if in.Mids == _all {
		rly.AllMids = true
	} else {
	...
	rly.BuildLimit = make(map[string][]*PlatformLimit, 0)
...
func InvalidBuild(srcBuild, cfgBuild int32, cfgCond string) bool {
	switch cfgCond {
	case _greaterThan:
