...
func (d *dao) getMsgKey(c context.Context, l *lottery.LetterParam) (res *msgKey, code int, err error) {
	...
	if err != nil {
		log.Errorc(c, "getMsgKey json.Marshal getMsgKeyURL(%s) params(%+v) error(%v)", getMsgKeyURL, params, err)
		return
	}
	...
	if req, err = http.NewRequest("POST", getMsgKeyURL, strings.NewReader(paramStr)); err != nil {
		log.Errorc(c, "getMsgKey http.NewRequest url(%s) error(%v)", getMsgKeyURL+"?"+paramStr, err)
		return
	}
	req.Header.Set("Content-Type", "application/json")
	if err = d.client.Do(c, req, &resp); err != nil {
		log.Errorc(c, "getMsgKey d.httpClient.Post getMsgKeyURL(%s) error(%v)", getMsgKeyURL+"?"+paramStr, err)
		return
	}
	if resp.Code != 0 {
		log.Errorc(c, "getMsgKey getMsgKeyURL(%s) error code(%v)", getMsgKeyURL+"?"+paramStr, resp.Code)
		return
	}
	log.Infoc(c, "getMsgKey success by getMsgKeyURL(%s)", getMsgKeyURL+"?"+paramStr)
	res = resp.Data
	code = resp.Code
	return
}
...
func (d *dao) SendLetter(c context.Context, l *lottery.LetterParam) (code int, err error) {
	var (
		msgKeyResp *msgKey
		msgKeycode int
	)
	for i := 1; i < 3; i++ {
		...
		case _whiteCheckError: //白名单校验失败，直接返回，程序继续往下执行
			...
			log.Errorc(c, "getMsgKey code(%d) msg(白名单校验失败) error(%v)", msgKeycode, err)
			return
		case _writeDBError:
			err = errors.New("写入数据库错误") //会导致获取msgkey失败，建议重试
			log.Errorc(c, "getMsgKey code(%d) msg(写入数据库错误) error(%v) retry 3 times", msgKeycode, err)
			continue
		}
	}
	if msgKeyResp == nil || msgKeyResp.MsgKey == 0 {
		log.Errorc(c, "getMsgKey error msg key(%+v) error", msgKeyResp)
		return
	}
