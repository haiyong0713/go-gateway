...
func (i *ShowItem) FromBangumiArchivePB(a *api.Arc, season *seasongrpc.CardInfoProto, bangumiType int) {
	...
	case _bangumiTVID:
		if season.NewEp != nil && season.NewEp.Id > 0 {
			epid := strconv.Itoa(int(season.NewEp.Id))
			i.URI = "bilibili_wild://pgc?season=" + i.Param + "&epid=" + epid
		} else {
			i.URI = "bilibili_wild://pgc?season=" + i.Param
		}
	}
	i.Goto = model.GotoBangumi
	i.Play = int(a.Stat.View)
...
// FromJobPBRegion
func (r *Region) FromJobPBRegion(reg *v1.Region) {
	r.ID = reg.Id
	r.Rid = int(reg.Rid)
	r.Reid = int(reg.Reid)
	r.Name = reg.Name
	...
	r.Param = reg.Param
	r.Rank = reg.Rank
	r.Plat = int8(reg.Plat)
	r.Area = reg.Area
	r.Language = reg.Language
	r.Build = int(reg.Build)
	r.Condition = reg.Condition
	r.URI = reg.Uri
	r.Islogo = int8(reg.IsLogo)
	r.Rtype = int8(reg.Rtype)
	r.Entrance = int8(reg.Entrance)
}

func (l *Limit) FromJobPBLimit(ltm *v1.Limit) {
	l.ID = ltm.Id
	l.Rid = ltm.Rid
	l.Build = int(ltm.Build)
	l.Condition = ltm.Condition
}

func (c *Config) FromJobPBConfig(cfg *v1.Config) {
	c.ID = cfg.Id
	c.Rid = cfg.Rid
	c.ScenesID = int(cfg.ScenesId)
	c.ScenesName = cfg.ScenesName
	c.ScenesType = cfg.ScenesType
	c.configChange()
}
