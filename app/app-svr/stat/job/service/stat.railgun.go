...
func (s *Service) statRailgunDo(ctx context.Context, item interface{}) railgun.MsgPolicy {
	...
	s.mu.Lock()
	if _, ok := s.subMonitor[stat.Type]; ok {
		s.subMonitor[stat.Type].Count++
	}
	s.mu.Unlock()
	...
	// 处理正常消息
	var err error
	if err = s.initRedisAndDB(ctx, stat); err != nil {
		return railgun.MsgPolicyFailure
	}
	if err = s.updateRedisAndDB(ctx, stat); err != nil {
		return railgun.MsgPolicyFailure
	}
	if err = s.arcRedisSync(ctx, stat.Aid); err != nil {
		log.Error("statRailgunDo redis sync error aid = %d", stat.Aid)
		return railgun.MsgPolicyFailure
	}
