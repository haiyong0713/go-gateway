...
func (s *Service) GenerateVer(ctx context.Context, resName string, limitData *model.Limit, fInfo *model.FileInfo, pool *model.ResourcePool, defPkg int) (resID int, version int, err error) {
	// 老表没有唯一索引，并发情况插入的mod会产生相同版本
	// 通过使用redis锁解决
	key := fmt.Sprintf("gen_ver_%d", pool.ID)
	locked, err := s.dao.TryLock(ctx, key, 5)
	if err != nil {
		log.Error("trylock error:%+v", err)
		return 0, 0, err
	}
	if !locked {
		log.Warn("trylock lock failed,%v,%+v", resName, pool)
		return 0, 0, errors.New("trylock lock failed")
	}
	defer func() {
		if err1 := s.dao.UnLock(ctx, key); err1 != nil {
			log.Error("unlock error:%+v", err1)
		}
	}()
