...
func (s *Service) reportV2(c context.Context, data *infoc, list string) error {
	payload := infocv2.NewLogStream(s.c.FeedInfocv2.LogID, data.now, list)
	if err := s.feedInfocv2.Info(c, payload); err != nil {
		log.Error("Fail to report FeedInfoc, data=%+v error=%+v", data, err)
		return err
	}
	return nil
}
