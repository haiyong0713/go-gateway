...
func GenerateChannelApk(out string, channel string, extras map[string]string, input string, force bool, debug bool) (output string, err error) {
	...
	verityPaddingBlock, _ := readVerityPaddingBlock(input)
	needPadding := verityPaddingBlock != nil

	err = gen(c, z, output, force, needPadding)
	if err != nil {
...
func gen(info ChannelInfo, sections zipSections, output string, force bool, needPadding bool) (err error) {
	...
	err = sections.writeTo(output, newTransform(info, needPadding))
	if _debug {
...
func newTransform(info ChannelInfo, needPadding bool) transform {
	return func(zip *zipSections) (*zipSections, error) {

		newBlock, diffSize, err := makeSigningBlockWithChannelInfo(info, zip.signingBlock, needPadding)
		if err != nil {
			return nil, err
		}
