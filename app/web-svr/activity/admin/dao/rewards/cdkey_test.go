...
`)
	...
	err := testDao.UploadCdKey(ctx, 1000, "test", keys)
	if !assert.Equal(t, nil, err) {
		t.FailNow()
	}
	err = testDao.SendCdKey(ctx, 216761, "test-1", "unique_id_error")
	assert.NotEqual(t, nil, err)
	for i := 0; i < 10000; i++ {
		err = testDao.SendCdKey(ctx, 216761, "test", fmt.Sprintf("unique_id_%v", i))
		assert.Equal(t, nil, err)
	}

	//out of stock
	err = testDao.SendCdKey(ctx, 216761, "test", "unique_id_error")
	assert.NotEqual(t, nil, err)

	err = testDao.UploadCdKey(ctx, 1000, "test-1", []string{"abc"})
	if !assert.Equal(t, nil, err) {
		t.FailNow()
	}
	err = testDao.SendCdKey(ctx, 88888, "test-1", fmt.Sprintf("unique_id_888"))
	assert.Equal(t, nil, err)
	res, err := testDao.GetCdKeyList(ctx, 88888, 50)
	assert.Equal(t, nil, err)
