...
func (s *Service) loadRcmmndCache(now time.Time) {
	...
	tmp, tmpOsea := s.fromAids(context.TODO(), aids, 0, "", "")
	if len(tmp) > 0 {
		s.rcmmndCache = tmp
	}
	if len(tmpOsea) > 0 {
...
func (s *Service) loadRegionCache(now time.Time) {
	...
	for _, tid := range _tids {
		...
		if err != nil || len(rs) < 4 {
			...
			innerArc := s.controld.CircleReqInternalAttr(context.Background(), aids)
			tmp[tid], tmpOsea[tid] = s.fromArchivesPB(res, innerArc)
			tmpBg[tid], tmpBgOsea[tid] = s.fromArchivesBangumi(context.TODO(), res, aids, sids, _bangumiSeasonID, innerArc)
			tmpBgEp[tid], tmpBgEpOsea[tid] = s.fromArchivesBangumi(context.TODO(), res, aids, sids, _bangumiEpisodeID, innerArc)
		} else {
			if _, isBangumi := _bangumiReids[tidInt]; isBangumi {
				sids, _ = s.fromSeasonID(context.TODO(), rs)
			}
			tmp[tid], tmpOsea[tid], tmpBg[tid], tmpBgOsea[tid], tmpBgEp[tid], tmpBgEpOsea[tid] =
				s.fromBgAids(context.TODO(), rs, sids, _bangumiSeasonID, 0, "", "")
		}
...
// loadBannerCache load banner cache.
func (s *Service) loadBannerCache() {
	var (
	...
	log.Info("loadBannerCache success")
}
...
func (s *Service) loadRankAllCache() {
	...
	if as, err = s.arc.ArchivesPB(c, rankAids, 0, "", ""); err != nil {
	...
	innerArc := s.controld.CircleReqInternalAttr(c, rankAids)
	s.rankArchivesCache = as
	tmp, tmpOsea := s.fromRankAids(c, rankAids, scores, as, innerArc)
	if len(tmp) > 0 {
		s.rankCache = tmp
	}
	if len(tmpOsea) > 0 {
	...
	if len(trankCache) > _max {
		s.rankCache2 = trankCache[:_max]
	} else {
...
// loadColumnListCache
func (s *Service) loadColumnListCache() {
	var (
		tmpChild = map[int]*card.ColumnList{}
	)
	columns, err := s.cdao.ColumnList(context.TODO())
	if err != nil {
...
func (s *Service) loadCardCache(now time.Time) {
	hdm, err := s.cdao.PosRecs(context.TODO())
	if err != nil {
		log.Error("s.cdao.PosRecs error(%v)", err)
		return
	}
	itm, aids, err := s.cdao.RecContents(context.TODO())
	if err != nil {
	...
	for recid, aid := range aids {
		tmpItem[recid] = s.fromCardAids(context.TODO(), aid, 0, "", "")
	}
...
// nolint:gomnd
func (s *Service) fillItem(_ int8, si *show.Item) {
	pp := strings.Split(si.Param, ",")
	si.Param = pp[0]
	if len(pp) == 2 {
		si.Goto = pp[1]
...
func (s *Service) loadBlackCache() {
	var res []int64
	if err := s.client.Get(context.TODO(), s.blackURL, "", nil, &res); err != nil {
		log.Error("recommend ranking url(%s) error(%v)", s.blackURL, err)
		return
	}
	if len(res) == 0 {
		return
...
func (s *Service) rcmmndproc() {
	...
	for i := 0; i < runtime.NumCPU(); i++ {
		...
		go func() {
			for {
				r := <-s.rcmmndCh
				_ = s.dao.AddRcmmndCache(ctx, r.key, r.aids...)
			}
		}()
	}
}
...
func (s *Service) PopularCardTenList(c context.Context, i, index, ps int) (res []*card.PopularCard) {
	var err error
	if res, err = s.dao.PopularCardTenCache(c, i, index, ps); err != nil {
		log.Error("日志告警 热门降级数据获取失败: %+v", err)
		return
	}
	return
}

func (s *Service) PopularEntranceAI(c context.Context, id int64) (res []*card.PopularCard) {
	var err error
	if res, err = s.dao.CacheAIChannelRes(c, id); err != nil {
		log.Error(" CacheAIChannelRes err:%+v, id:%d", err, id)
	}
	if len(res) > _maxAids {
		res = res[:_maxAids]
	}
...
func (s *Service) loadArticleCardsCache() {
	cards, err := s.dao.ArticleCard(context.TODO())
	if err != nil {
		log.Error("%+v", err)
		return
	}
	s.articleCards = cards
}
