...
// ComicUser def.
func (d *Dao) ComicUser(c context.Context, mid int64) (comicUser bool, err error) {
	var res struct {
		Code int    `json:"code"`
		Msg  string `json:"message"`
		Data bool   `json:"data"`
	...
	req, err := http.NewRequest("POST", d.comicUserURL, bytes.NewReader(bytesData))
	if err != nil {
		err = errors.Wrap(err, d.comicUserURL+"?"+string(bytesData))
		return false, err
	}
	req.Header.Set("Content-Type", "application/json;charset=UTF-8")
	if err := d.client.Do(c, req, &res); err != nil {
	...
	if res.Code != 0 {
		err = errors.Wrap(ecode.Int(res.Code), d.comicUserURL+"?"+string(bytesData))
		return false, err
	}
	return res.Data, nil
}
