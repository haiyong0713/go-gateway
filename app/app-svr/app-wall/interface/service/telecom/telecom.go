...
func (s *Service) TelecomPay(c context.Context, phone, isRepeatOrder, payChannel, payAction int, orderID int64, ipStr string) (res *telecom.Pay, msg string, err error) {
	...
	if err = s.dao.AddPayPhone(c, requestNo, phoneStr); err != nil {
...
func (s *Service) sendSMS(c context.Context, phone int, msg string) (err error) {
	var (
		captcha string
	)
	...
	if err = s.dao.SendSMS(c, phone, msg, dataJSON); err != nil {
		log.Error("telecom_s.dao.SendSMS error (%v)", err)
		return
	}
	if err = s.dao.AddPhoneCode(c, phone, captcha); err != nil {
...
func (s *Service) OrderState(c context.Context, orderid int64) (t *telecom.OrderState, msg string, err error) {
	...
	g.Go(func() error {
		if orderState, err = s.dao.OrderState(ctx, orderid); err != nil {
			log.Error("telecom_s.dao.OrderState orderID:%d error:%+v", orderid, err)
			return err
		}
		return nil
	})
	g.Go(func() error {
		if torder, err = s.telecomInfoByOrderID(c, orderid); err != nil {
			log.Error("telecom_s.telecomInfoByOrderID:%d error:%+v", orderid, err)
			return err
		}
...
func (s *Service) iplimit(ip string) bool {
	addrs := s.c.IPLimit.Addrs[_telecomKey]
	if len(addrs) == 0 {
		return true
	}
	log.Info("isp:%v,ip limit list:%+v", _telecomKey, addrs)
	for _, addr := range addrs {
		if ip == addr {
			return true
		}
	}
	log.Error("日志告警 IP白名单检验不通过,isp:%v,ip:%v", _telecomKey, ip)
	return false
}
