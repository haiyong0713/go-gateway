...
func (d *dao) EsSearchChannel(c context.Context, mid int64, keyword string, pn, ps, state int) (st *search.ChannelResult, tids []int64, err error) {
	var (
		req     *http.Request
	...
	var res struct {
		...
		Data struct {
			...
			Result []*search.SearchChannel `json:"result"`
			Page   struct {
				Num   int `json:"num"`
				Size  int `json:"size"`
