...
func (s *Service) catchFile(file *model.ResourceFile, version int) (newPath string, err error) {
	...
	if newPkg, err = s.getFile(file.ResourceID, _fullPackage, version); err != nil {
		log.Error(errFormat, "catchFile ", fmt.Sprintf("getFile %v", file), err)
		_ = s.chgFStatus(file.ID, _diffPackge)
		return
	}
	...
	if size, err = s.dao.DownloadFile(ctx, newPkg.URL, newPath); err != nil {
		log.Error(errFormat, "catchFile", fmt.Sprintf("DownloadFile %v", file), err)
		_ = s.chgFStatus(file.ID, _diffPackge)
		return
	}
	// check result file
	if _, err = os.Stat(newPath); os.IsNotExist(err) {
		log.Error(errFormat, "catchFile", "IsNotExist", newPath+" - File Not Exist")
		return
	}
	log.Info("catchFile Save File From URL [%s] in [%s], Size: %d", newPkg.URL, newPath, size)
	return
}
...
func (s *Service) calDiff(file *model.ResourceFile) (err error) {
	...
	platForm, err := s.dao.Platform(ctx, file.ResourceID)
	if err != nil {
		log.Error(errFormat, "callDiff", "Platform", err)
		return
	}
	...
	if patchFInfo, patchURL, err = s.diffCmd(file.Name, patchPath, newPath, oldPath, platForm); err != nil {
		log.Error(errFormat, "calDiff", "diffCmd", err)
		_ = s.chgFStatus(file.ID, _diffPackge)
		return
	}
	log.Info("Upload Path File From [%s] to [%s]", patchPath, patchURL)
...
func (s *Service) diffCmd(patchName string, patchPath string, newPath string, oldPath, platForm string) (patchFInfo *model.FileInfo, location string, err error) {
	var (
		content []byte
		fInfo   os.FileInfo
	)
	...
	log.Info("Patch File Generated, Name: %s, Size: %d.", fInfo.Name(), fInfo.Size())
	...
	patchFInfo.Name = rename(patchName, patchFInfo.Md5, platForm)
	c := context.Background()
	if _, err = s.dao.UploadBoss(c, patchFInfo.Name, content); err != nil {
		log.Error(errFormat, "diffCmd", "UploadBoss", err)
		return
	}
	location = s.c.Host.Boss + "/" + model.BossBucket + "/" + patchFInfo.Name
	return
}

// split the patchName, pick the ModID (res[0]), VersionInfo (res[1]), Insert the md5 inside
// nolint:gomnd
func rename(patchName string, md5, platForm string) (newName string) {
	...
	return res[0] + "_" + md5 + "/" + platForm + "/" + res[1]
}
