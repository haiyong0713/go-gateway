...
func tab(c *bm.Context) {
	...
	c.JSON(favSvr.Tab(c, param))
}

func secondTab(c *bm.Context) {
	param := &struct {
		Tab string `form:"tab" default:"special_topic" validate:"required"`
	}{}
	if err := c.Bind(param); err != nil {
		return
	}
	...
	c.JSON(favSvr.SecondTab(c, param.Tab, mid), nil)
}
...
func channel(c *bm.Context) {
	...
	c.JSON(favSvr.ChannelFav(c, mid, param.Ps, param.Offset, "main.my-favorite.0.0"))
}
