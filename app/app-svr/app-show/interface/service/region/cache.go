...
import (
	...
	"go-gateway/app/app-svr/app-show/interface/model/tag"
...
var (
	// 番剧 动画，音乐，舞蹈，游戏，科技，娱乐，鬼畜，电影，时尚, 生活，国漫，动物圈
	_tids = []int{13, 1, 3, 129, 4, 36, 5, 119, 23, 155, 160, 11, 167, 217}
)
...
func (s *Service) loadShowChildTagsInfo() {
	...
	for _, v := range res {
		if v.Reid != 0 {
			...
			if tags := s.loadShowChildTagIDs(v.Rid); len(tags) > 0 {
				var tidm = make(map[int64]int64)
				for _, tag := range tags {
					tidm[tag.Tid] = tag.Tid
				}
				var tids []int64
				for tid := range tidm {
				...
				if channels, err = s.chDao.Infos(context.Background(), tids, 0); err != nil {
					log.Error("%v", err)
					err = nil
				}
				for _, tag := range tags {
					...
					if channels != nil {
						if channel, ok := channels[tag.Tid]; ok && channel != nil {
							if channel.CType == model.NewChannel {
								tagInfo.URI = model.FillURI(model.GotoChannelNewAll, strconv.FormatInt(tag.Tid, 10), nil)
							}
						}
					}
					if rTmp != nil {
...
func (s *Service) loadShowHot(rid int) (resData, resOseaData []*region.ShowItem) {
	...
	resData, resOseaData = s.fromAids(context.TODO(), res, false, 0, 0, "", "")
...
func (s *Service) loadShowNewRpc(rid int) (resData, resOseaData []*region.ShowItem) {
	...
	for _, v := range arcs {
		...
		arcAids = append(arcAids, v.Aid)
	}
	innerArc := s.controld.CircleReqInternalAttr(context.Background(), arcAids)
	resData, resOseaData = s.fromArchivesPB(arcs, innerArc)
...
func (s *Service) loadShowChileHot(rid int) (resData, resOseaData []*region.ShowItem, arcAids []int64) {
	...
	resData, resOseaData = s.fromAids(context.TODO(), arcAids, false, 0, 0, "", "")
...
func (s *Service) loadShowChildNew(rid int) (resData, resOseaData []*region.ShowItem, arcAids []int64) {
	...
	arcs, arcAids, err = s.circleReqArcs(rid, 300)
...
func (s *Service) loadRankRegionCache() {
	...
	for _, rid := range _tids {
		...
		tRank, tOseaRank := s.fromRankAids(context.TODO(), aids, others, scores, 0, "", "")
		tmp[rid] = tRank
				...
				case 2:
					if topicID, err := strconv.ParseInt(hd.Rvalue, 10, 64); err == nil {
						if actm, err := s.act.Activitys(c, []int64{topicID}, _topic, ""); err != nil {
							log.Error("s.act.Activitys topicID(%d) error (%v)", topicID, err)
						} else {
...
func (s *Service) circleReqArcs(rid, total int) (arcs []*api.Arc, arcAids []int64, err error) {
	var (
		arcMap = make(map[int64]*api.Arc, total)
		maxPn  = int(math.Ceil(float64(total)/float64(_maxAids))) + 1
	)
	for pn := 1; pn <= maxPn; pn++ {
		var arcRes []*api.Arc
		if arcRes, _, err = s.arc.RanksArcs(context.Background(), rid, pn, _maxAids); err != nil {
			log.Error("[circleReqArcs] s.arc.RanksArcs() rid(%d) error(%v)", rid, err)
			return
		}
		if len(arcRes) == 0 {
			return
		}
		for _, v := range arcRes { // aid去重
			if _, ok := arcMap[v.Aid]; v.IsNormal() && !ok {
				arcMap[v.Aid] = v
				arcs = append(arcs, v)
				arcAids = append(arcAids, v.Aid)
			}
