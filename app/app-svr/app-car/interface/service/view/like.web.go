...
func (s *Service) LikeWeb(c context.Context, mid int64, buvid, path, ua string, param *view.LikeWebParam) (err error) {
	var (
		a   *api.SimpleArc
		typ thumbup.Action
	)
	if a, err = s.arc.SimpleArc(c, param.Oid); err != nil {
	...
	if _, err = s.thumbupDao.Like(c, mid, a.Mid, _businessLike, a.Aid, typ, true, model.AndroidBilithings, "", "android"); err != nil {
...
func (s *Service) CommunityWebPGC(c context.Context, mid int64, buvid string, param *view.CommunityWebParam) (*view.Community, error) {
	pgcAv, err := s.bgm.AvInfo(c, int32(param.Cid))
	if err != nil {
		log.Error("%+v", err)
		return nil, err
	}
