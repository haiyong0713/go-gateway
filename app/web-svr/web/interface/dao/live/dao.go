...
func (d *Dao) RoomByIds(c context.Context, roomIds []int64) (res map[int64]*model.LiveRoomInfo, err error) {
	args := &livegrpc.EntryRoomInfoReq{
		RoomIds:    roomIds,
		EntryFrom:  []string{"NONE"},
		NotPlayurl: 1,
		ReqBiz:     "/x/web-interface/top/feed/rcmd",
	}
	...
	res = make(map[int64]*model.LiveRoomInfo, len(roomIds))
...
func packingInfo(list *livegrpc.EntryRoomInfoResp_EntryList) *model.LiveRoomInfo {
	if list == nil {
		return nil
	}
	info := &model.LiveRoomInfo{
		RoomId:     list.RoomId,
		Uid:        list.Uid,
		LiveStatus: list.LiveStatus,
