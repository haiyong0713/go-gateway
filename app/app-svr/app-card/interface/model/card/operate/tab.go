...
func (a *Active) Change() {
	...
	switch a.Type {
	// 基本类型
	case "player_live", "converge", "special", "article_s", "player", "player_ogv":
		var id int64
	...
	case "entrance", "banner":
		...
		for _, item := range card.CardItem {
			...
			ri := &Active{Goto: model.OperateType[typ], Pid: id, Param: item.LinkValue, Title: item.Title, Cover: item.Cover, BigCover: item.BigCover}
			if ri.Goto != "" {
				ris = append(ris, ri)
			}
		}
