...
func wrapError(reply statusGetter) error {
	return CastTaishanError(reply.GetStatus())
}

func (ts *Taishan) initialTest(ctx context.Context) {
	const (
		notFound = 404
	)
	req := ts.NewGetReq([]byte("__initialtest_key_dangerous__"))
	reply, err := ts.client.Get(ctx, req)
	if err != nil {
		panic(err)
	}
	if err := CastTaishanError(reply.Record.Status); err != nil {
		tsErr, ok := errors.Cause(err).(*TaishanError)
		if !ok {
			panic(err)
		}
		if tsErr.ErrNo == notFound {
			return
		}
		panic(err)
	}
...
func NewKV() (*Taishan, func(), error) {
	...
	taishan.initialTest(context.Background())
	cf := func() {}
...
func CastTaishanError(status *taishan.Status) error {
	if status == nil {
...
func (ts *Taishan) NewBatchGetReq(ctx context.Context, keys []string) *taishan.BatchGetReq {
	records := make([]*taishan.Record, 0, len(keys))
...
func (ts *Taishan) BatchGet(ctx context.Context, req *taishan.BatchGetReq) (*taishan.BatchGetResp, error) {
	reply, err := ts.client.BatchGet(ctx, req)
	if err != nil {
		return nil, err
	}
	return reply, nil
