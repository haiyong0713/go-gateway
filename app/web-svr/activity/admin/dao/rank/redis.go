...
// SetRank 设置排名
func (d *dao) SetRank(c context.Context, rankNameKey string, rankBatch []*rankmdl.MidRank) (err error) {
	if len(rankBatch) == 0 {
		return
	}
	var (
...
// GetRank 设置排名
func (d *dao) GetRank(c context.Context, rankActivityKey string) (res []*rankmdl.MidRank, err error) {
	var (
		bs   []byte
		key  = buildKey(rankActivityKey, rankKey)
		conn = d.redis.Get(c)
	)
	res = make([]*rankmdl.MidRank, 0)
	defer conn.Close()
...
func (d *dao) SetMidRank(c context.Context, rankNameKey string, midRank []*rankmdl.MidRank) (err error) {
	if len(midRank) == 0 {
		return
	}
	conn := d.redis.Get(c)
	...
	count := 1
	for _, v := range keys {
		if err := conn.Send("EXPIRE", v, rankExpire); err != nil {
			log.Errorc(c, "SetMidRank conn.Send(Expire, %s, %d) error(%v)", v, rankExpire, err)
			return err
		}
		count++
	}
