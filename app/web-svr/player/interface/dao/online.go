...
func (d *Dao) OnlineCount(c context.Context, aid, cid int64) (count int64, err error) {
	...
	if err = d.client.Get(c, d.onlineCountURL, ip, params, &res); err != nil {
		log.Error("OnlineCount d.client.Get(%s) error(%v)", d.onlineCountURL, err)
		return
	}
	if res.Code != ecode.OK.Code() {
		log.Error("OnlineCount d.client.Get(%s) code(%d) error", d.onlineCountURL, res.Code)
		return
	}
	if len(res.Data) == 0 || res.Data[0].Cid != cid {
		log.Error("OnlineCount d.client.Get(%s) data(%v) error", d.onlineCountURL, res.Data)
	}
