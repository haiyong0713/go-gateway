...
type Categories []*article.Category
...
type Draft struct {
	*article.Article
	...
	List   *article.List `json:"list"`
}

// Metas Metas
type Metas []*article.Meta
...
type CreationArts struct {
	Articles []*article.Meta   `json:"articles"`
...
type ViewInfo struct {
	...
	Coin            int64         `json:"coin"`
	Stats           article.Stats `json:"stats"`
...
type RecommendHome struct {
	RecommendPlus
	Categories []*article.Category `json:"categories"`
	IP         string              `json:"ip"`
}
...
func NoDistributeAttr(attr int32) bool {
	meta := article.Meta{Attributes: attr}
...
type ImageCheckResp struct {
	...
	IsPolitics   bool    `json:"is_politics"`
}
