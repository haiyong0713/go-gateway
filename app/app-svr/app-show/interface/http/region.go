...
func regionsIndex(c *bm.Context) {
	...
	data, version, err := regionSvc.NewRegionList(c, plat, build, teenagersMode, ver, mobiApp, device, language)
	if err == ecode.NotModified {
		c.JSON(nil, err)
		return
	}
...
func regionShow(c *bm.Context) {
	...
	// GetAudit
	if audit, ok := regionSvc.Audit(c, mobiApp, plat, build, rid, true, mid, device); ok {
...
func regionChildShow(c *bm.Context) {
	...
	if tagIDStr != "" {
		if tagID, err = strconv.Atoi(tagIDStr); err != nil {
			log.Error("tagId(%d) error(%v)", tagID, err)
			c.JSON(nil, ecode.RequestErr)
			return
		}
	}
	...
	plat := model.Plat(mobiApp, device)
	// GetAudit
	if audit, ok := regionSvc.AuditChild(c, mobiApp, "default", plat, build, rid, tagID, mid, device); ok {
		returnJSON(c, audit, nil)
	} else {
		mobiApp = model.MobiAPPBuleChange(mobiApp)
		data := regionSvc.ChildShow(c, plat, mid, rid, tagID, build, channel, mobiApp, time.Now(), device)
		returnJSON(c, data, nil)
	}
}
...
func regionShowDynamic(c *bm.Context) {
	...
	plat := model.Plat(mobiApp, device)
	data := regionSvc.ShowDynamic(c, plat, build, rid, pn, ps, mid, mobiApp, device)
	returnJSON(c, data, nil)
}
...
func regionDynamicList(c *bm.Context) {
	...
	// GetAudit
	if _, ok := regionSvc.Audit(c, mobiApp, plat, build, rid, false, mid, device); ok {
		data := map[string]interface{}{}
		returnJSON(c, data, nil)
	} else {
		mobiApp = model.MobiAPPBuleChange(mobiApp)
		data := regionSvc.DynamicList(c, plat, rid, pull, ctime, mid, time.Now(), mobiApp, device)
		returnJSON(c, data, nil)
	}
}
...
func regionDynamicChild(c *bm.Context) {
	...
	// GetAudit
	if audit, ok := regionSvc.AuditChild(c, mobiApp, "", plat, build, rid, tagID, mid, device); ok {
		returnJSON(c, audit, nil)
	} else {
		mobiApp = model.MobiAPPBuleChange(mobiApp)
		data := regionSvc.DynamicChild(c, plat, rid, tagID, build, mid, mobiApp, time.Now(), device)
		returnJSON(c, data, nil)
	}
}
...
func regionDynamicChildList(c *bm.Context) {
	...
	if audit, ok := regionSvc.AuditChild(c, mobiApp, "", plat, build, rid, tagID, mid, device); ok {
		...
		mobiApp = model.MobiAPPBuleChange(mobiApp)
		data := regionSvc.DynamicListChild(c, plat, rid, tagID, build, pull, ctime, mid, time.Now(), mobiApp, device)
		returnJSON(c, data, nil)
	}
}
