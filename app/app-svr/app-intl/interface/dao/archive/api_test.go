...
func TestRelateAids(t *testing.T) {
	Convey(t.Name(), t, func() {
		var aid = int64(43850845)
		d.client.SetTransport(gock.DefaultTransport)
		httpMock("GET", d.realteURL).Reply(200).JSON(`{
			"code": 0,
			"data": [
				{
					"key": "43850845",
					"value": "29707096,30298721,28056901,67457647,22958914,29240625,27892473,45762366,23031001,24600482,43738028,43788369,29925856,39034080,71517245,41520341,43456016,34160485,45312766,44891162,7396850,44164956,51660059,49491180,38887090,53464783,41433717,45440726,33216057,10493338,26704963,56597226,66992322,44023071,59165605,43002233,23955312"
				}
			]
		}`)
		res, err := d.RelateAids(context.Background(), aid)
...
func TestNewRelateAids(t *testing.T) {
	Convey(t.Name(), t, func() {
		var (
			aid, mid             int64
			build, autoplay      int
			buvid, from, trackid string
			plat                 int8
		)
		d.client.SetTransport(gock.DefaultTransport)
		httpMock("GET", d.relateRecURL).Reply(200).JSON(`{
			"code": 0,
			"data": [
				{
		...
		res, _, err := d.NewRelateAids(context.Background(), aid, mid, build, autoplay, buvid, from, trackid, plat, 0)
		So(res, ShouldNotBeNil)
		So(err, ShouldBeNil)
	})
}
