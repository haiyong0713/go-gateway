...
func TransformArticle(arg *ArgArticle) *Article {
	a := &Article{
		Meta: &article.Meta{
			ID:              arg.Aid,
			Category:        &article.Category{ID: arg.Category},
			Title:           arg.Title,
			Summary:         arg.Summary,
			BannerURL:       arg.BannerURL,
			TemplateID:      arg.TemplateID,
			State:           arg.State,
			Author:          &article.Author{Mid: arg.Mid},
			Reprint:         arg.Reprint,
			ImageURLs:       arg.ImageURLs,
			OriginImageURLs: arg.OriginImageURLs,
			Words:           arg.Words,
			Dynamic:         arg.DynamicIntro,
			Media:           &article.Media{MediaID: arg.MediaID, Spoiler: arg.Spoiler},
			Original:        arg.Original,
		},
		Content: arg.Content,
	}
	for _, t := range arg.Tags {
		a.Tags = append(a.Tags, &article.Tag{Name: t})
	}
	return a
}
