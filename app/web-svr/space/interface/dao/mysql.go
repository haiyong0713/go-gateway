...
func (d *Dao) ChannelMid(ctx context.Context, channelID int64) (mid, cid int64, err error) {
	_channelSQL := "SELECT mid FROM member_channel%d WHERE id = ?"
	cid = channelID / _chSub
	if cid == 0 {
		return 0, 0, nil
	}
	row := d.db.QueryRow(ctx, fmt.Sprintf(_channelSQL, channelHit(channelID)), cid)
