...
const (
	...
	AsgTypeAv      = int8(6)
	AsgTypeTopic   = int8(7)
	AsgTypeOGVPay  = int8(9)
	AsgTypeWebLive = int8(10)
	// content type
	FromManager = int8(0)
	FromCpm     = int8(1)
	// activity type
...
type ArgRess struct {
	...
	IP        string
	Buvid     string
	IsNotAD   bool   `form:"is_not_ad"`
	Aid       int64  `form:"aid"`
	BVID      string `form:"bvid"`
	UserAgent string `form:"-"`
	FromSpmID string `form:"from_spm_id"`
}
...
type FrontPage struct {
	...
	IsSplitLayer int32      `json:"is_split_layer"`
	SplitLayer   string     `json:"split_layer"`
	Style        int32      `json:"style"`
	STime        xtime.Time `json:"stime"`
...
func (f *FrontPage) FormFrontPage(fs *resourcegrpc.FrontPage) {
	f.ID = fs.GetId()
	f.Title = fs.GetTitle()
	f.Pos = fs.GetPos()
	f.Logo = fs.GetLogo()
	f.Litpic = fs.GetLitpic()
	f.JumpURL = fs.GetJumpUrl()
	f.State = fs.GetState()
	f.STime = fs.GetStime()
	f.ETime = fs.GetEtime()
	f.IsSplitLayer = fs.GetIsSplitLayer()
	f.SplitLayer = fs.GetSplitLayer()
	f.Style = fs.GetStyle()
	f.ResID = fs.GetResourceId()
}
