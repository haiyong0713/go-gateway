...
// SubInfos
func TestService_SubInfos(t *testing.T) {
	Convey("sub SubInfos ", t, WithService(func(s *Service) {
		list, err := s.SubInfos(context.Background(), []int64{1412, 10256, 10257})
		So(err, ShouldBeNil)
		fmt.Printf("%+v", list)
	}))
}

func TestService_AddNativePage(t *testing.T) {
	Convey("sub AddNativePage ", t, WithService(func(s *Service) {
		isNotice := s.AddNativePage(context.Background(), "测试native page")
		So(isNotice, ShouldBeTrue)
		fmt.Printf("AddNativePage testapi %+v", isNotice)
	}))
}
