...
func (d *Dao) NoticeUser(mids []int64, body string, contest *mdlesp.Contest) (err error) {
	...
	res := &_response{}
	if err = d.http.Do(context.Background(), req, &res); err != nil {
		log.Error("httpClient.Do() error(%v)", err)
		return
	}
	if res.Code != 0 || res.Data == 0 {
		log.Error("push failed matchID(%d) mids_total(%d) body(%s) response(%+v)", contest.ID, len(mids), body, res)
	} else {
		log.Info("push success matchID(%d) mids_total(%d) body(%s) response(%+v)", contest.ID, len(mids), body, res)
	}
	return
}
