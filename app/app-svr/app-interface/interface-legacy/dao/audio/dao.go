...
const (
	...
	_music       = "/x/internal/v1/audio/songs/batch"
)
...
func New(c *conf.Config) (d *Dao) {
	d = &Dao{
		...
		card:        c.HostDiscovery.Music + _card,
		fav:         c.Host.APICo + _fav,
		music:       c.Host.APICo + _music,
	}
	return
}
...
func (d *Dao) MusicMap(c context.Context, musicIds []int64) (data map[int64]*audio.Music, err error) {
	params := url.Values{}
	params.Set("level", "1")
	params.Set("ids", xstr.JoinInts(musicIds))
	var res struct {
		Code int                    `json:"code"`
		Data map[int64]*audio.Music `json:"data"`
	}
	ip := metadata.String(c, metadata.RemoteIP)
	if err = d.client.Get(c, d.music, ip, params, &res); err != nil {
		return
	}
	if res.Code != ecode.OK.Code() {
		err = errors.Wrap(ecode.Int(res.Code), d.fav+"?"+params.Encode())
