...
func TestAddCoins(t *testing.T) {
	Convey(t.Name(), t, func() {
		var (
			aid, mid, upID, maxCoin, avtype, multiply int64
			typeID                                    int16
			pubTime                                   int64
		)
		err := d.AddCoins(context.Background(), aid, mid, upID, maxCoin, avtype, multiply, typeID, pubTime)
...
func TestArchiveUserCoins(t *testing.T) {
	Convey(t.Name(), t, func() {
		var (
			aid, mid, avtype int64
		)
		res, err := d.ArchiveUserCoins(context.Background(), aid, mid, avtype)
		Convey("Then err should be nil.res should not be nil.", func() {
...
func TestUserCoins(t *testing.T) {
	Convey(t.Name(), t, func() {
		...
		res, err := d.UserCoins(context.Background(), mid)
		Convey("Then err should be nil.res should not be nil.", func() {
			So(res, ShouldNotBeNil)
			So(err, ShouldBeNil)
		})
