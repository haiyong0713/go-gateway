...
func (d *Dao) SendWeChart(c context.Context, ns int8, userName string, res []*model.ResWarnInfo, titleType string) (err error) {
	...
	req, _ := http.NewRequest("POST", d.c.WeChantDomain, payload)
...
func (d *Dao) sendWeChatURL(_ context.Context, code int, userName string, res []*model.ResWarnInfo) (err error) {
	var (
		users    = append(d.c.WeChantUsers, userName)
		codeInfo string
		contents []string
