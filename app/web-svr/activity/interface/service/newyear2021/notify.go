...
func (s *Service) getMsgKey(c context.Context, senderId uint64, msgContent string) (res *msgKey, code int, err error) {
	getMsgKeyURL := s.c.Host.Dynamic + "/biz_msg_svr/v0/biz_msg_svr/get_msg_key" //业务方请求获取msg_key
	params := make(map[string]interface{})
	params["sender_uid"] = senderId                                       //发送方uid
	params["msg_type"] = 1                                                //消息类型：文本类型 type = 1，通知卡片10
	ctn, _ := json.Marshal(map[string]interface{}{"content": msgContent}) //消息实体内容
...
func (s *Service) SendLetter(c context.Context, mids []int64, msgContent string) (err error) {
	senderId := 37090048
	var (
		msgKeyResp *msgKey
		msgKeycode int
	...
	for i := 1; i < 3; i++ {
		msgKeyResp, msgKeycode, err = s.getMsgKey(c, uint64(senderId), msgContent)
	...
	params := make(map[string]interface{})
	params["sender_uid"] = senderId                              //TODO:发送方uid
	params["recver_ids"] = mids                                  //TODO:多人消息，列表型，限定每次客户端发送100个
	params["msg_key"] = msgKeyResp.MsgKey                        //TODO:消息唯一标识
	params["jump_text"] = "测试跳转链接"                               //TODO
	params["jump_uri"] = "https://uat-space.bilibili.com/216761" //TODO
