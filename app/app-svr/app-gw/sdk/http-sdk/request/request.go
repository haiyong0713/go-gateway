...
type Request struct {
	...
	MetricURI    string
	Data         interface{}
	DataBytes    []byte // DataBytes is the all bytes read from response.
...
func New(cfg sdk.Config, clientInfo metadata.ClientInfo, handlers Handlers,
...
func (r *Request) prepareRetry() error {
	log.Info(fmt.Sprintf("DEBUG: Retrying Request %s/%s, attempt %d",
		r.ClientInfo.AppID, r.Operation.Name, r.RetryCount))
	...
	r.ResetBody()
	if err := r.Error; err != nil {
		return errors.WithStack(sdkerr.New(ErrCodeSerialization,
...
func (r *Request) getNextRequestBody() (body io.ReadCloser, err error) {
	...
	if err != nil {
		return nil, errors.WithStack(sdkerr.New(ErrCodeSerialization,
...
func WithMetricURI(metricURI string) Option {
	return func(r *Request) {
		r.MetricURI = metricURI
	}
}
...
func (r *Request) WillRetry() bool {
	if !sdk.IsReaderSeekable(r.Body) && r.HTTPRequest.Body != NoBody {
		return false
	}
...
func (r *Request) SetStreamingBody(reader io.ReadCloser) {
	...
	r.SetReaderBody(sdk.ReadSeekCloser(reader))
}
...
// SetClientInfo is
func (r *Request) SetClientInfo(clientInfo metadata.ClientInfo) error {
	var err error
	r.HTTPRequest.URL, err = url.Parse(clientInfo.Endpoint + r.Operation.HTTPPath)
	if err != nil {
		r.HTTPRequest.URL = &url.URL{}
		err = errors.WithStack(sdkerr.New(ErrCodeRequestError, fmt.Sprintf("invalid endpoint url: %s", clientInfo.Endpoint+r.Operation.HTTPPath), err))
	}
	r.ClientInfo = clientInfo
	if r.ClientInfo.Timeout > 0 {
		// 配置时录入的是ms
		//nolint
		t := xtime.Duration(r.ClientInfo.Timeout) * xtime.Duration(time.Millisecond)
...
func isValidResponse(r *Request) bool {
...
func (r *Request) SafeMetricURI() string {
	if isValidResponse(r) {
		return r.MetricURI
	}
	return r.ClientInfo.Identifier()
}
...
func debugLogReqError(r *Request, stage, retryStr string, err error) {
	// The design is so. Skip the ecode.
	if _, ok := errors.Cause(err).(ecode); ok {
		return
	}
	log.Info(fmt.Sprintf("DEBUG: %s %s/%s failed, %s, error %+v",
		stage, r.ClientInfo.AppID, r.Operation.Name, retryStr, err))
}
