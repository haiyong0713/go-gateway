...
func ranking(c *bm.Context) {
	var (
		...
		items []*webmdl.Mi
		v     = new(struct {
			Rid    int    `form:"rid"     validate:"required"`
	...
	if !permission(v.Source) {
		c.JSON(nil, ecode.AccessDenied)
		return
	}
	source := "?bsource=" + v.Source
	if items, err = srvWeb.RankingReg(c, v.Rid, v.Day, source); err != nil {
		c.JSON(nil, err)
		return
	}
	data := make(map[string]interface{}, 5)
...
func permission(user string) bool {
	for _, v := range conf.Conf.OutSearch.RankReg {
		if user == v {
			return true
		}
	}
