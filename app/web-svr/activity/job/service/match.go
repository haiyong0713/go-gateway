	...
	for {
		...
		for _, v := range list {
			if v.Result == matchObj.Result {
				...
				if _, err = s.coinClient.ModifyCoins(context.Background(), &api.ModifyCoinsReq{Mid: v.Mid, Count: count, Reason: _reason}); err != nil {
					log.Error("upMatchUser coin error s.coinClient.ModifyCoins mid(%d) coin(%v) error(%v)", v.Mid, count, err)
					continue
				}
				log.Info("upMatchUser s.coinClient.ModifyCoins mid(%d) coin(%v)", v.Mid, count)
