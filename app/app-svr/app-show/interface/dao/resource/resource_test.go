...
func TestResBanner(t *testing.T) {
	Convey("ResBanner", t, func() {
		var (
			...
			build                                                           int
			mid                                                             int64
			resIDStr, channel, ip, buvid, network, mobiApp, device, adExtra string
			isAd                                                            bool
		)
		res, err := d.ResBanner(ctx(), plat, build, mid, resIDStr, channel, ip, buvid, network, mobiApp, device, adExtra, isAd)
