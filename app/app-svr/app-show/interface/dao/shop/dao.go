...
const (
	...
	_addFavURI  = "/api/ticket/user/addfavinner"
	_delFavURI  = "/api/ticket/user/delfavinner"
)
...
func New(c *conf.Config) (d *Dao) {
	d = &Dao{
		...
		addFavURL:  c.Host.ShowCo + _addFavURI,
		delFavURL:  c.Host.ShowCo + _delFavURI,
	}
	return
}
...
func (d *Dao) AddFav(c context.Context, itemID, mid int64) error {
	ip := metadata.String(c, metadata.RemoteIP)
	params := url.Values{}
	...
	if err := d.client.Post(c, d.addFavURL, ip, params, &res); err != nil {
		return err
	}
	if res.Code != ecode.OK.Code() {
		err := errors.Wrap(ecode.Int(res.Code), d.addFavURL+"?"+params.Encode())
		return err
	}
	return nil
}

func (d *Dao) DelFav(c context.Context, itemID, mid int64) error {
	ip := metadata.String(c, metadata.RemoteIP)
	params := url.Values{}
	params.Set("item_id", strconv.FormatInt(itemID, 10))
	params.Set("mid", strconv.FormatInt(mid, 10))
	...
	if err := d.client.Post(c, d.delFavURL, ip, params, &res); err != nil {
		return err
	}
	if res.Code != ecode.OK.Code() {
		err := errors.Wrap(ecode.Int(res.Code), d.delFavURL+"?"+params.Encode())
		return err
	}
	return nil
}
