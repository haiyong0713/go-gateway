	...
	for _, v := range s.c.Bnj2019.Info {
		if v.Publish.Unix() < now {
			if arc, ok := arcs[v.Aid]; ok && arc.IsNormal() {
				...
				tmpArc := *arc
				tmpArc.Pic = strings.Replace(tmpArc.Pic, "http://", "//", 1)
				if v.Nickname != "" {
					tmpArc.Author.Name = tmpArc.Author.Name + "&" + v.Nickname
				}
				tmp.Arc = &tmpArc
...
func (s *Service) Timeline(c context.Context) *bnj.Timeline {
...
func (s *Service) TimeReset(c context.Context, mid int64) (ttl int64, err error) {
	...
	if !value {
		if ttl, err = s.dao.TTLResetCD(c, mid); err != nil {
			log.Error("TimeReset s.dao.TTLResetCD(%d) error(%v)", mid, err)
			err = nil
		}
		return
	}
