...
const (
	...
	_diffPkg      = 1
	_pool         = "SELECT pool_id FROM resource WHERE id = ?"
	_platform     = "SELECT platform FROM resource_pool WHERE id = ?"
)
...
// Platform
func (d *Dao) Platform(ctx context.Context, id int) (string, error) {
	var (
		poolID   int64
		platForm string
	)
	row := d.db.QueryRow(ctx, _pool, id)
	err := row.Scan(&poolID)
	if err != nil {
		log.Error("Platform pool (%d) error(%v)", id, err)
		return "", err
	}
	row = d.db.QueryRow(ctx, _platform, poolID)
	err = row.Scan(&platForm)
	if err != nil {
		log.Error("Platform platform (%d) error(%v)", id, err)
		return "", err
	}
	return platForm, nil
}
