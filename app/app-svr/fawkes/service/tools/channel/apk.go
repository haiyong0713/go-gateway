...
const (
	...
	verityPaddingBlockID            = 0x42726577
	androidCommonPageAlignmentBytes = 4096
)
...
// read block associated to verityPaddingBlockID
func readVerityPaddingBlock(file string) ([]byte, error) {
	m, err := readIDValues(file, verityPaddingBlockID)
	if err != nil {
		return nil, err
	}
	return m[verityPaddingBlockID], nil
}
...
// uint64:  size (same as the one above)
// uint128: magic
func makeSigningBlockWithChannelInfo(info ChannelInfo, signingBlock []byte, needPadding bool) ([]byte, int, error) {
	...
	var padding int
	if needPadding {
		if resultSize%androidCommonPageAlignmentBytes != 0 {
			padding = androidCommonPageAlignmentBytes - int(resultSize%androidCommonPageAlignmentBytes)
			//nolint:gomnd
			if padding < 12 {
				padding += androidCommonPageAlignmentBytes
			}
			resultSize += uint64(padding)
		}
	}
	...
	if needPadding {
		putUint64(uint64(padding-8), newBlock, position)
		position += 8
		putUint32(verityPaddingBlockID, newBlock, position)
		position += 4
		//nolint:gomnd
		position = position + padding - 12
	}

	putUint64(resultSize-8, newBlock, position)
	position += 8
	_, _ = copyBytes(signingBlock, signingBlockLen-16, newBlock, int(resultSize-16), 16)
	position += 16
