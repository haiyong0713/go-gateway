...
func TestAppMRoom(t *testing.T) {
	Convey("AppMRoom", t, func() {
		...
		var (
			roomids                                                                                []int64
			mid                                                                                    int64
			platform, deviceName, accessKey, actionKey, appkey, device, mobiApp, statistics, buvid string
			build, teenagersMode, appver, filtered, httpsUrlReq                                    int
		)
		res, err := d.AppMRoom(ctx(), roomids, mid, platform, deviceName, accessKey, actionKey, appkey, device, mobiApp, statistics, buvid, build, teenagersMode, appver, filtered, httpsUrlReq)
...
func TestFeedList(t *testing.T) {
	Convey("FeedList", t, func() {
		...
		res, _, err := d.FeedList(ctx(), mid, pn, ps)
		So(res, ShouldNotBeEmpty)
		So(err, ShouldBeNil)
	})
}
