...
var (
	...
	greyAid  int64 = 1008600 // 灰度命中
	missAid  int64 = 1008601 // 灰度miss
	testStat       = &api.Stat{
		Aid:     testAid,
		View:    80,
		Danmaku: 200,
...
func Test_Stat_Redis_Insert_And_Retrieve(t *testing.T) {
	Convey("testConnRedisAndInsert", t, func() {
		conn := s.statRedis.Get(context.TODO())
...
func Test_Flush_Redis_To_DB(t *testing.T) {
	Convey("Test_Flush_Redis_To_DB", t, func() {
		stat := testStat
		testAid := greyAid
		testStat.Aid = testAid
		err := s.saveStatToRedis(context.Background(), stat)
		So(err, ShouldBeNil)
		queryStat0, err := s.getStatFromRedis(context.TODO(), testAid)
		fmt.Printf("stat redis: %+v", queryStat0)
		err = s.flushRedisToDB(context.TODO(), testAid)
		So(err, ShouldBeNil)
		queryStat, err := s.dao.Stat(context.Background(), testAid)
		So(queryStat, ShouldResemble, stat)
	})
}
...
func Test_Marshal_And_Unmarshal(t *testing.T) {
	Convey("Test_Marshal_And_Unmarshal", t, func() {
		stat := api.Stat{
			Aid:     10010,
			Like:    366,
			Danmaku: 366,
		...
		fmt.Printf("encoded size: %d", len(item))
		recovered := new(api.Stat)
		if err = recovered.Unmarshal(item); err != nil {
			t.Fail()
			log.Error("%v", err)
		}
		So(stat.Like, ShouldEqual, recovered.Like)
	})
}
