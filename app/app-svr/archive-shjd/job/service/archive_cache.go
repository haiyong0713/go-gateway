...
func (s *Service) setArcCache(c context.Context, arc *api.Arc) error {
	...
	if arc.AttrVal(api.AttrBitIsCooperation) == api.AttrYes {
		staff, err := s.dao.Staff(c, arc.Aid)
		if err != nil {
			return err
		}
		arc.StaffInfo = staff
	...
	desc, descV2, subType, err := s.dao.RawAdditV2(c, arc.Aid)
	if err != nil && err != sql.ErrNoRows {
		log.Error("RawAdditV2 arc(%+v) error(%+v)", arc, err)
		return err
	}
	//设置付费属性
	...
	if err != nil {
		log.Error("setArcCache Marshal arc(%+v) error(%+v)", arc, err)
		return nil
	}
	...
	log.Warn("setArcCache setArc k:%s v:%+v", arcmdl.ArcKey(arc.Aid), arc)

	resultAddit := &archive.Addit{
		Aid:         arc.Aid,
		Description: desc,
	}
	if resultAddit.Description == "" {
		resultAddit.Description = arc.Desc
	}
	resultAddit.DescV2 = descV2
...
func (s *Service) setVideosPageCache(c context.Context, aid int64, vs []*api.Page) error {
	...
	bs, err := v.Marshal()
	if err != nil {
		log.Error("setVideosPageCache Marshal v(%+v) err(%+v) ", v, err)
		return nil
	}
	...
	log.Warn("setVideosPageCache setPage k:%s v:%+v", arcmdl.PageKey(aid), v)
	return nil
}

func (s *Service) initStatCache(c context.Context, aid int64) (err error) {
	if _, err = s.dao.Stat(c, aid); err != nil {
...
func (s *Service) setVideoCache(c context.Context, aid, cid int64, p *api.Page) error {
	bs, err := p.Marshal()
	if err != nil {
		log.Error("setVideoCache Marshal error(%+v) aid(%d) cid(%d)", err, aid, cid)
		return nil
	}
...
func (s *Service) getArcPremiereInfo(c context.Context, aid int64) (*api.Premiere, error) {
	expand, err := s.dao.RawArchiveExpand(c, aid)
	if err != nil {
		return nil, err
	}
	if expand == nil {
...
func (s *Service) setArcPayInfo(c context.Context, sa *api.Arc) error {
	if sa.SeasonID != 0 && sa.Pay.AttrVal(api.PaySubTypeAttrBitSeason) == api.AttrYes {
		episode, err := s.dao.RawSeasonEpisode(c, sa.SeasonID, sa.Aid)
		if err != nil {
			return err
		}
		if episode == nil {
...
func (s *Service) setSimpleArcPayInfo(c context.Context, sa *api.SimpleArc) error {
	_, _, subType, err := s.dao.RawAdditV2(c, sa.Aid)
	if err != nil && err != sql.ErrNoRows {
		return err
	}
	sa.Pay = &api.PayInfo{
