...
import (
	...
	"go-gateway/app/app-svr/app-show/interface/model/activity"
...
type HotConfig struct {
	...
	BottomTextCover string          `json:"bottom_text_cover,omitempty"`
	BottomTextURL   string          `json:"bottom_text_url,omitempty"`
	TopItems        []*EntranceShow `json:"top_items,omitempty"` // top items distributes the entries of popular index
	ShareInfo       ShareInfo       `json:"share_info,omitempty"`
}

type ShareInfo struct {
	CurrentTopPhoto string `json:"current_top_photo"`
...
type EntranceShow struct {
	...
	ShareInfo  ShareInfo `json:"-"`
}

// Bubble .
type Bubble struct {
	BubbleContent string `json:"bubble_content"`
...
// GoodHisRes is good history resource card
...
// FromJobPBHead
func (h *Head) FromJobPBHead(head *v1.Head) {
	h.ID = int(head.Id)
	h.Plat = int8(head.Plat)
	h.Param = head.Param
	h.Type = head.Type
	h.Style = head.Style
	h.Title = head.Title
	h.Rank = int(head.Rank)
	h.Build = int(head.Build)
	h.Condition = head.Condition
	h.Language = head.Language
}

// FromJobPBItem
func (i *Item) FromJobPBItem(item *v1.Item) {
	i.Sid = int(item.Sid)
	i.Title = item.Title
	i.Cover = item.Cover
	i.Param = item.Param
	i.Random = int(item.Random)
}
