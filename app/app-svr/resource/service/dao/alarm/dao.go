...
func (d *Dao) CheckURL(originURL string, wis []*model.ResWarnInfo) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		for userName, send := range sends {
			if err := d.sendWeChatURL(context.TODO(), resp.StatusCode, userName, send); err != nil {
				log.Error("sendWeChartURL StatusCode(%d) userName(%s) send(%v) error(%v)", resp.StatusCode, userName, send, err.Error())
			}
		}
	}
}
