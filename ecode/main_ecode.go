...
var (
	...
	UpdateUnameSensitive      = New(40002) // 昵称包含敏感信息
	...
	MemberSignSensitive       = New(40015) // 签名包含敏感信息
	...
	// SilverBullet
	SilverBulletLikeReject = New(137004) //账号异常。点赞失败
	SliverBulletFavReject  = New(137006) // 账号异常。收藏失败
	SilverBulletCoinReject = New(137007) // 账号异常。投币失败

	// Activity season
	WhiteListErr           = New(77810) // 白名单拦截
	PlatActivityUnExistErr = New(77811) // 活动未配置

	// web-svr
	WebSLBRetry = New(77300) // Web HTTP 接口，SLB 重试
	// app-svr
	AppSLBRetry = New(77301) // App HTTP 接口，SLB 重试

	// story teenager code
	StoryTeenagersModeErr = New(77302)

	// deeplink
	DeeplinkTimeoutErr = New(77400) // ai deeplink接口获取超时
