...
#     echo "Docker deamon start failed, find more detail in stdout."
#     exit 1
# fi
...
function GetPackages(){
    ...
    for dir in ${utdirs[@]}
    do
        reg+="|app/(.+)/(.+)/(service|interface|admin|job)/(internal/)?${dir}/(.*).go"
    done
...
function GoTest(){
    ...
    while [ ! $testPath = "app" ]
    do
        if [[ -s $GOPATH/src/${REPO_NAME}/$testPath/test/docker-compose.yaml || -s $GOPATH/src/${REPO_NAME}/$testPath/test/docker-compose.yml ]];then
                break
        fi
        testPath=${testPath%/*}
    done
    if [[ $testPath = "app" ]];then
        echo "No docker-compose.yaml or docker-compose.yml found under any path of $APP_ID"
        exit 1
    fi
    testcli -f $GOPATH/src/${REPO_NAME}/$testPath/test/docker-compose.yaml run go test -v -gcflags=-l $1 -coverprofile=cover.out -covermode=set -convey-json -timeout=60s > result.out
