...
import (
	...
	"go-gateway/app/app-svr/app-channel/interface/model/dislike"
...
type ChannelInfo struct {
	...
	Item        []*ChannelInfoItem `json:"items,omitempty"`
}
...
func (i *Item) FromPlayerAv(ap *api.ArcPlayer) {
	var a = ap.Arc
	if i.Title == "" {
		i.Title = a.Title
	}
	...
	i.Goto = model.GotoAv
	i.URI = model.FillURI(i.Goto, i.Param, 0, 0, 0, cdm.ArcPlayHandler(a, cdm.ArcPlayURL(ap, 0), "", nil, 0, "", true))
	i.Cid = a.FirstCid
	i.Rid = a.TypeID
	i.TName = a.TypeName
...
func (i *Item) FromConverge(c *operate.Converge, am map[int64]*api.ArcPlayer, rm map[int64]*cardlive.Room, artm map[int64]*article.Meta) {
	...
	for _, content := range c.Items {
		...
		switch content.Goto {
		case model.GotoAv:
			if ap, ok := am[content.Pid]; ok && ap != nil && ap.Arc != nil {
				var a = ap.Arc
				if a.IsNormal() {
					if ci.Title == "" {
						ci.Title = a.Title
					...
					ci.URI = model.FillURI(ci.Goto, ci.Param, 0, 0, 0, cdm.ArcPlayHandler(a, nil, "", nil, 0, "", false))
					ci.fillArcStat(a)
...
func (i *Item) FromPlayer(ap *api.ArcPlayer) {
	var a = ap.Arc
	if !a.IsNormal() {
		return
	}
	...
	item.URI = model.FillURI(item.Goto, item.Param, 0, 0, 0, cdm.ArcPlayHandler(a, cdm.ArcPlayURL(ap, 0), "", nil, 0, "", true))
	item.fillArcStat(a)
	i.Item = []*Item{item}
	i.Cid = a.FirstCid
	i.Rid = a.TypeID
...
func (i *Item) FromLiveUpRcmd(id int64, cs []*cardlive.Card, card map[int64]*accountgrpc.Card) {
	// nolint:gomnd
	if len(cs) < 2 {
		return
	}
...
func (i *Item) FromSubscribe(r *operate.Follow, card map[int64]*accountgrpc.Card, follow map[int64]bool, upStatm map[int64]*relationgrpc.StatReply, tagm map[int64]*taggrpc.Tag) {
	if r == nil {
		return
	}
	is := make([]*Item, 0, 3)
...
func (i *Item) FromChannelRcmd(r *operate.Follow, am map[int64]*api.ArcPlayer, tagm map[int64]*taggrpc.Tag) {
	if r == nil {
		return
	}
	if ap, ok := am[r.Pid]; ok && ap != nil {
		var a = ap.Arc
		i.Goto = model.GotoChannelRcmd
		i.URI = model.FillURI(model.GotoAv, strconv.FormatInt(a.Aid, 10), 0, 0, 0, cdm.ArcPlayHandler(a, nil, "", nil, 0, "", false))
		i.Title = a.Title
		i.Cover = a.Pic
...
func (i *ChannelInfoItem) FromChannelInfoItem(r *operate.Card) {
	i.Goto = string(r.Goto)
	i.Param = r.Param
	i.URI = model.FillURI(model.GotoAv, strconv.FormatInt(r.ID, 10), 0, 0, 0, nil)
	i.FromType = r.FromType
}
