...
func TestService_UploadInfo(t *testing.T) {
	Convey("UploadGray", t, WithService(func(svf *Service) {
		fInfo, err := svf.UploadGray(context.Background(), []byte("{\"test\":\"test\"}"))
		So(err, ShouldBeNil)
		fmt.Println(fInfo)
	}))
}
...
func TestService_Upload(t *testing.T) {
	Convey("Upload should return without err", t, WithService(func(svf *Service) {
		...
		data2 := []byte(str2)
		// upload to bfs
		location, err := svf.UploadBfs(c, "", "image/png", now, data2)
		// testing
		So(err, ShouldBeNil)
