...
func TestLikeRawActSubjects(t *testing.T) {
	convey.Convey("RawActSubjects", t, func(ctx convey.C) {
		var (
			c   = context.Background()
			ids = []int64{10308}
		)
		ctx.Convey("When everything goes positive", func(ctx convey.C) {
			res, err := d.RawActSubjects(c, ids)
			ctx.Convey("Then err should be nil.res should not be nil.", func(ctx convey.C) {
				ctx.So(err, convey.ShouldBeNil)
				ctx.So(res, convey.ShouldNotBeNil)
			})
...
func TestAddSubAwardLog(t *testing.T) {
	convey.Convey("AddSubAwardLog", t, func(ctx convey.C) {
		var (
			c = context.Background()
		)
		ctx.Convey("When everything goes positive", func(ctx convey.C) {
			err := d.AddSubAwardLog(c, 250, "", 1, 2089809)
			ctx.So(err, convey.ShouldBeNil)
		})
	})
}
