...
func (s *Service) showDisplay(c context.Context, mid int64, plat int8, build int, buvid, channel, ip, ak, network, mobiApp,
	...
	for i, sw := range ss {
		...
		if sw.Type == "recommend" {
			...
			if isRegion || s.c.Custom.LiveRcmdClose {
				continue
			}
...
func (s *Service) LiveChange(c context.Context, mid int64, ak, ip string, rand int, now time.Time) (sis []*show.Item, err error) {
	if s.c.Custom.LiveRcmdClose {
		return nil, ecode.RequestErr
	}
	return s.getLive(c, mid, ak, ip, rand, now), nil
}
...
Retry:
	for i = 0; i < 2; i++ {
		if aids, err = s.dao.PopRcmmndCache(ctx, key, cnt); err != nil {
			log.Error("s.dao.PopRcmmndCache(%s) error(%v)", key, err)
			return
		}
		if len(aids) < cnt {
			break
		...
		if sis = s.fromAidsOsea(ctx, aids, isOsea, mid, mobiApp, device); len(sis) < cnt {
	...
	if sis = s.fromAidsOsea(ctx, caids, isOsea, mid, mobiApp, device); len(sis) < clen { // NOTE: if cnt=1 means dislike change one
		for {
			var (
				over  = cnt - len(sis)
				start = 0
			...
			if tmp := s.fromAidsOsea(ctx, aids[start:over], isOsea, mid, mobiApp, device); len(tmp) > 0 {
				sis = append(sis, tmp...)
			}
		}
		return
...
func (s *Service) apiRecommend(ctx context.Context, plat int8, key, _ string, _ int64) (rcs []*rcmmndCfg, aids []int64) {
	var (
		uri    string
		recURL = s.c.Host.Data + "/mobile/home/%s"
	)
...
func (s *Service) dealLive(_ context.Context, sw *show.Show, sis []*show.Item) (rs *show.Show) {
	rs = &show.Show{
		Head: sw.Head,
		Body: sis,
		Ext:  sw.Ext,
...
func (s *Service) fromArchivesPB(as []*api.Arc, innerArc map[int64]*rankmdl.InnerAttr) (sis, sisOsea []*show.Item) {
	var asLen = len(as)
	if asLen == 0 {
		sis = []*show.Item{}
		return
...
func (s *Service) fromArchivesBangumi(c context.Context, as []*api.Arc, aids []int64, sids map[int32]*seasongrpc.CardInfoProto, bangumiType int, innerArc map[int64]*rankmdl.InnerAttr) (sis, sisOsea []*show.Item) {
	var (
		asLen = len(as)
		err   error
		// bangumi
	...
	for _, a := range as {
		i := &show.Item{}
		if sid, ok := sids[int32(a.Aid)]; ok && sid != nil && sid.SeasonId != 0 {
...
func (s *Service) fromArchivesOsea(as []*api.Arc, isOsea bool) (sis []*show.Item) {
	tmp, tmpOsea := s.fromArchivesPB(as, nil)
	if isOsea {
		sis = tmpOsea
	} else {
		sis = tmp
...
func (s *Service) fromArchivesBangumiOsea(c context.Context, as []*api.Arc, aids []int64, isOsea bool, bangumiType int) (sis []*show.Item) {
	tmp, tmpOsea := s.fromArchivesBangumi(c, as, aids, nil, bangumiType, nil)
	if isOsea {
		sis = tmpOsea
	} else {
		sis = tmp
...
// fromAids get Aids.
func (s *Service) fromAids(ctx context.Context, aids []int64, mid int64, mobiApp, device string) (sis, sisOsea []*show.Item) {
	var (
	...
	eg.Go(func(ctx context.Context) (err error) {
		if as, err = s.arc.ArchivesPB(ctx, aids, mid, mobiApp, device); err != nil {
	...
	for _, aid := range aids {
		...
		if v, ok := as[aid]; ok {
			if inva, k := innerArc[aid]; k && inva != nil && inva.OverSeaBlock {
				isOverseas = int32(1)
			}
			...
			si.URI = model.AvHandler(v)(si.URI)
		}
		if isOverseas == 0 {
...
func (s *Service) fromCardAids(ctx context.Context, aids []int64, mid int64, mobiApp, device string) (sis map[int64]*show.Item) {
...
// fromRankAids
func (s *Service) fromRankAids(_ context.Context, aids []int64, scores map[int64]int64, as map[int64]*api.Arc, innerArc map[int64]*rankmdl.InnerAttr) (sis, sisOsea []*show.Item) {
	var (
		aid int64
		arc *api.Arc
		ok  bool
	...
	for _, aid = range aids {
		if arc, ok = as[aid]; ok {
			...
			if inva, k := innerArc[arc.Aid]; k && inva != nil {
...
// fromAids get Aids.
func (s *Service) fromBgAids(ctx context.Context, aids []int64, sids map[int32]*seasongrpc.CardInfoProto, _ int, mid int64, mobiApp, device string) (sis, sisOsea, sisbg, sisbgOsea, sisbgep, sisbgepOsea []*show.Item) {
	var (
		err error
	)
	as, err := s.arc.ArchivesPB(ctx, aids, mid, mobiApp, device)
	...
	for _, aid := range aids {
		...
		sibgep := &show.Item{}
		if v, ok := as[aid]; ok {
			if innerArc != nil && innerArc[aid] != nil && innerArc[aid].OverSeaBlock {
				isOverseas = 1
			}
			if sid, ok := sids[int32(aid)]; ok && sid != nil && sid.SeasonId != 0 {
...
// isOverseas
func (s *Service) fromAidsOsea(ctx context.Context, aids []int64, isOsea bool, mid int64, mobiApp, device string) (sis []*show.Item) {
	tmp, tmpOsea := s.fromAids(ctx, aids, mid, mobiApp, device)
	if isOsea {
		sis = tmpOsea
	} else {
		sis = tmp
