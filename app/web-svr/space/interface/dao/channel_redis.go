...
func (d *Dao) DelChannelCache(c context.Context, mid, cid int64) (err error) {
	var (
		...
		midKey  = channelMidKey(cid*10 + mid%10)
		conn    = d.redis.Get(c)
	)
	defer conn.Close()
	...
	if err = conn.Send("DEL", midKey); err != nil {
		log.Error("conn.Send(DEL,%s) error(%v)", midKey, err)
	}
	if err = conn.Flush(); err != nil {
...
func channelMidKey(channelID int64) string {
	return fmt.Sprintf("clmid_%d", channelID)
}

func (d *Dao) ChannelMidCache(ctx context.Context, channelID int64) (mid, cid int64, err error) {
	conn := d.redis.Get(ctx)
	defer conn.Close()
	key := channelMidKey(channelID)
	...
	return data["mid"], data["cid"], nil
}

func (d *Dao) SetChannelMidCache(ctx context.Context, channelID, mid, cid int64) error {
	conn := d.redis.Get(ctx)
	defer conn.Close()
	key := channelMidKey(channelID)
	v := map[string]int64{
	...
	_, err = conn.Do("SETEX", key, d.clExpire, bs)
	return err
}
...
func (d *Dao) DelChannelsCache(c context.Context, mid int64) (err error) {
	conn := d.redis.Get(c)
	defer conn.Close()
	_, err = conn.Do("DEL", keyCl(mid))
	return
}
